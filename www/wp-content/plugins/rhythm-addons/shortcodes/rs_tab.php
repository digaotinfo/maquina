...
function rs_tabs( $atts, $content = '', $id = '' ) {
  ...
  foreach( $rs_tabs as $key => $tab) {
    ...
    $tab_image  = (isset($tab['atts']['image'])) ? wp_get_attachment_image_src( $tab['atts']['image'], 'full' ):'';
    $active_nav = ( ( $key + 1 ) == $active ) ? ' class="active"' : '';
   
    switch ( $style ){
    	case "with_icon":
		    $icon       = ($tab_icon) ? '<div class="alt-tabs-icon"><span class="'.sanitize_html_classes($tab_icon).'"></span></div>'.esc_html($title):'';
    		break;
    	
    	case "with_image":
    		$imgView = '<div class="iconTab"><img src="' .$tab_image[0]. '"></div>';
		    $icon       = ($tab_image) ? '<div class="alt-tabs-icon img">' .$imgView. '</div>':'';
//     		$imgView = '<div class="iconTab" style="background: url('.$tab_image[0].')"></div>';
// 		    $icon       = ($tab_image) ? '<div class="alt-tabs-icon img"><div class="iconTab" style="background: url('.$tab_image[0].')"></div> </div>':'';
    		break;
    }
    $output     .=  '<li '.$active_nav.'><a href="#'.$tab_id.esc_attr($uniqtab).'-'.$key.'" data-toggle="tab">' .$icon. '</a></li>';
  }
